...
def multi_run():
    ...
    s = xenv.reset()
    res_store_dir = "long_pos/result/F-3L-6M" + time.strftime("-%Y-%m-%d-%H-%M-%S", time.localtime())
    ...
    for i in range(MAX_EPISODES):
        ...
        while(True):
            ...
            if done :
              ...
              print('F-Ep: %i | %s | ep_r: %.1f | steps: %i |table :%i |memory :%i |stop_num :%i' % (i, '----' if not done else 'done', ep_r, j,len(ddpg.table),ddpg.memory_size,lanjie_num))
              s = xenv.reset()
