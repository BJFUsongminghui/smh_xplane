...
def multi_run():
    ...
    s = xenv.reset()
    res_store_dir = "long_pos/result/long-3L-6M-sub5" + time.strftime("-%Y-%m-%d-%H-%M-%S", time.localtime())
    ...
    for i in range(MAX_EPISODES):
        ...
        while(True):
            ...
            s = s_
            if done :
              print('sub-Ep: %i | %s | ep_r: %.1f | steps: %i | memory:%.1f' % (i, '----' if not done else 'done', ep_r, j,ddpg.memory_size))
              s = xenv.reset()
