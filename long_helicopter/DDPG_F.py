...
def getFeature(s,a):
    ...
    cur_key=str(round(low_dim_data[0][0],2))+'_'+str(round(low_dim_a[0][0],3))
    # print(s,' ',a,' ',cur_key)
    return cur_key
...
class DDPG(object):
    def __init__(self, s_dim, a_dim,memory_size=30000):
        self.memory_cap=memory_size
        self.a_dim, self.s_dim = a_dim, s_dim
        self.memory = np.zeros((self.memory_cap, s_dim * 2 + a_dim + 1 + 1), dtype=np.float32)
        self.pointer = 0
    ...
    def learn(self):
        ...
        if self.pointer > self.memory_cap:
            indices = np.random.choice(self.memory_cap, size=BATCH_SIZE)
        else:
    ...
    def store_transition(self, s, a, r, s_, done=False):
        ...
        if self.memory_size>= self.memory_cap:
            ...
            self.memory_size=self.memory_cap
        ...
        self.addTable(self.memory[self.pointer,  :self.s_dim],self.memory[self.pointer, self.s_dim: self.s_dim + self.a_dim])
        self.pointer += 1
        self.memory_size+=1
        self.pointer = self.pointer % self.memory_cap  # replace the old memory with new memory
        return 0
