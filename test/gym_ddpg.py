...
class Agent(object):
    def __init__(self, **kwargs):
        ...
        self.noise = 0.05
    def act(self, s0,explore=True):
        s0 = torch.tensor(s0, dtype=torch.float).unsqueeze(0)
        a0 = self.actor(s0).squeeze(0).detach()
        if explore:
            a0 = a0 + self._sample_exploration_noise(a0)
        return a0.numpy()

    def _sample_exploration_noise(self, action):
        mean = torch.zeros(action.size())
        var = torch.ones(action.size())
...
for episode in range(100):
    ...
    write.add_scalar('every_reward',episode_reward/step_num,episode)
    write.add_scalar('memory',len(agent.buffer),episode)
    print(episode, ': ', episode_reward,'    step :',step_num,'  noise:',agent.noise,'  memory:',len(agent.buffer))
